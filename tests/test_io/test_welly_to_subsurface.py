...
def test_read_lith():
    d = pd.read_excel(data_path + 'borehole_lith.xlsx')
    print(d)
    return d
...
def test_read_collars():
    d = pd.read_excel(data_path+'borehole_collar.xlsx',
...
def test_read_data_loc():
    ...
    attri = s.to_log(step_size, dev['DEPTH'].min()+step_size/2, dev['DEPTH'].max()-step_size/2)
    ...
    # s.read_at(dev['DEPTH'])
    unstructured_data = wts.to_subsurface(n_points=1001, attributes= attri)
