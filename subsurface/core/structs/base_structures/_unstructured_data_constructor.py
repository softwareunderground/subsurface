...
def _vertex_and_cells_arrays_to_data_array(cells: Union[np.ndarray, Literal["lines", "points"], SpecialCellCase],
                                           vertex: np.ndarray):
    n_vertex = vertex.shape[0]
    if type(cells) is np.ndarray:
        cells = _create_default_cells_arg(
            cells=cells,
            n_vertex=n_vertex
        )
...
def _raw_attributes_to_dict_data_arrays(
